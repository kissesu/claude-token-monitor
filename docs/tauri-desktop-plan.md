...
应用采用单窗口多页面设计，通过侧边栏导航切换页面。共包含 **3 个核心页面**：
...
| 页面 | 路由 | 功能描述 | 实现阶段 |
|------|------|----------|----------|
| 仪表盘 | `/` | 核心数据展示、图表可视化 | Phase 4, 6 |
| 供应商 | `/providers` | 供应商管理、统计对比 | Phase 5 |
| 设置 | `/settings` | 应用设置、主题、自启动 | Phase 7 |

---

### 3.6.2 页面 1: 仪表盘 (Dashboard)

**页面路径**: `/`（默认首页）

**核心职责**: 展示 Token 使用统计的核心数据和可视化图表
...
│  │  ──────────────────┼───────────┼──────────┼─────────┼─────────││
│  │  claude-opus-4     │ 450,000   │ 120,000  │ 67.3%   │ $32.50  ││
│  │  claude-sonnet-4   │ 280,000   │ 85,000   │ 72.1%   │ $8.40   ││
│  │  claude-haiku      │ 95,000    │ 25,000   │ 45.2%   │ $1.20   ││
│  └─────────────────────────────────────────────────────────────────┘│
│                        ModelUsagePanel 组件                          │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
| 模块 | 组件 | 功能描述 |
|------|------|----------|
| 统计概览 | `StatsOverview` | 4 个 StatCard 卡片，显示核心指标和环比变化 |
| 趋势图表 | `TrendChart` | 折线图展示 Token 使用量随时间变化趋势 |
| 模型占比 | `ModelPieChart` | 环形图展示各模型 Token 使用占比 |
| 费用分析 | `CostChart` | 堆叠柱状图按模型展示每日费用 |
| 活动热力图 | `ActivityHeatmap` | GitHub 风格热力图展示历史活动密度 |
| 模型详情 | `ModelUsagePanel` | 表格详细展示每个模型的使用统计 |

#### 数据源

| 数据 | Tauri Command | 更新触发 |
|------|---------------|----------|
| 统计概览 | `get_current_stats` | `stats-updated` 事件 |
| 模型用量 | `get_current_stats.models` | `stats-updated` 事件 |
| 每日活动 | `get_daily_activities` | 日期范围变化时 |

---

### 3.6.3 页面 2: 供应商管理 (Providers)

**页面路径**: `/providers`

**核心职责**: 管理多供应商 API Key，对比各供应商使用情况
...
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐        ││
│  │  │ 工作账号      │  │ 个人账号      │  │ 测试账号      │        ││
│  │  │ sk-ant-...abc │  │ sk-ant-...def │  │ sk-ant-...xyz │        ││
│  │  │               │  │               │  │               │        ││
│  │  │ Token: 450K   │  │ Token: 120K   │  │ Token: 25K    │        ││
│  │  │ 费用: $32.50  │  │ 费用: $8.40   │  │ 费用: $1.20   │        ││
│  │  │ 缓存: 67.3%   │  │ 缓存: 72.1%   │  │ 缓存: 45.2%   │        ││
│  │  │               │  │               │  │               │        ││
│  │  │ [✏️ 编辑名称] │  │ [✏️ 编辑名称] │  │ [✏️ 编辑名称] │        ││
│  │  └───────────────┘  └───────────────┘  └───────────────┘        ││
│  │              ProviderStatsCard 组件 × N                          ││
│  └──────────────────────────────────────────────────────────────────┘│
│                        TodayOverview 组件                            │
│                                                                      │
│  ────────────────────────────────────────────────────────────────   │
...
│  ┌─ 供应商详情 ────────────────────────────────────────────────────┐│
│  │                                                                  ││
│  │  API Key 前缀: sk-ant-api03-xxxx...                              ││
│  │  Base URL: https://api.anthropic.com                             ││
│  │  首次使用: 2024-01-15 14:30                                      ││
│  │  最后使用: 2024-03-08 16:45                                      ││
│  │  总会话数: 1,234                                                  ││
│  │  总 Token: 12.5M                                                  ││
│  │  总费用: $456.78                                                  ││
│  │                                                                  ││
...
| 模块 | 组件 | 功能描述 |
|------|------|----------|
| 供应商选择 | `ProviderSelector` | 下拉选择当前查看的供应商 |
| 今日概览 | `TodayOverview` | 卡片网格展示今日各供应商统计对比 |
| 统计卡片 | `ProviderStatsCard` | 单个供应商的核心指标展示 |
| 名称编辑 | 内联编辑 | 双击编辑供应商自定义显示名称 |
| 供应商详情 | 详情面板 | 选中供应商的完整信息展示 |

#### 数据源

| 数据 | Tauri Command | 更新触发 |
|------|---------------|----------|
| 供应商列表 | `get_providers` | `provider-switched` 事件 |
| 今日统计 | `get_today_provider_stats` | 供应商选择变化 |
| 名称更新 | `update_provider_name` | 用户编辑后提交 |

---

### 3.6.4 页面 3: 设置 (Settings)

**页面路径**: `/settings`

**核心职责**: 应用配置管理、主题切换、系统集成设置
...
│  │                                                                  ││
│  │  开机自启动                                        [  开关  ]    ││
│  │  系统启动时自动运行 Claude Token Monitor                         ││
│  │                                                                  ││
│  │  ────────────────────────────────────────────────────────────   ││
│  │                                                                  ││
│  │  最小化到托盘                                      [  开关  ]    ││
...
│  │                                                                  ││
│  │  数据存储位置                                                    ││
│  │  ~/Library/Application Support/com.claude-token-monitor/         ││
│  │                                          [ 打开文件夹 ]          ││
│  │                                                                  ││
│  │  ────────────────────────────────────────────────────────────   ││
│  │                                                                  ││
...
│  ┌─ 关于 ──────────────────────────────────────────────────────────┐│
│  │                                                                  ││
│  │  Claude Token Monitor v1.0.0                                     ││
│  │  基于 Tauri 2.x + React 19 构建                                  ││
│  │                                                                  ││
│  │  [ 检查更新 ]    [ GitHub ]    [ 反馈问题 ]                      ││
│  │                                                                  ││
│  └──────────────────────────────────────────────────────────────────┘│
...
| 模块 | 组件/功能 | 功能描述 |
|------|-----------|----------|
| 外观设置 | `ThemeToggle` | 主题模式切换（浅色/深色/系统） |
| 开机自启动 | `AutostartToggle` | 使用 `tauri-plugin-autostart` 控制 |
| 托盘行为 | `MinimizeToTrayToggle` | 控制窗口关闭行为 |
| 系统通知 | `NotificationToggle` | 使用 `tauri-plugin-notification` |
| 数据位置 | `DataLocationInfo` | 显示数据目录并提供打开按钮 |
| 数据清除 | `ClearDataButton` | 确认对话框后清除本地数据 |
| 关于信息 | `AboutPanel` | 版本信息、外部链接 |

#### 数据源

| 数据 | 来源 | 持久化 |
|------|------|--------|
| 主题偏好 | `useThemeStore` | localStorage |
| 自启动状态 | `@tauri-apps/plugin-autostart` | 系统级 |
| 托盘行为 | `useSettingsStore` | localStorage + Rust 状态 |
| 通知设置 | `useSettingsStore` | localStorage |
...
> **注意**：Tauri 桌面应用使用 React Router 实现客户端路由，无需服务端支持。
