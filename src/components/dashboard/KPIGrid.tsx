...
export function KPIGrid() {
  const { stats, todayStats, dailyActivities } = useAppStore();
  const latestActivity = dailyActivities.length > 0 ? dailyActivities[dailyActivities.length - 1] : null;

  return (
    <div className="grid grid-cols-4 gap-4">
      <div className="bento-card p-6 flex flex-col justify-between h-[8.75rem] animate-slide-up delay-100 group">
        <div className="flex justify-between items-start">
          <span className="font-mono text-[10px] uppercase tracking-wider text-secondary">今日费用</span>
        ...
        <div>
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-display font-medium text-white">
              ¥{todayStats ? (todayStats.cost_usd * 7.2).toFixed(2) : '0.00'}
            </span>
          </div>
      ...
      <div className="bento-card p-6 flex flex-col justify-between h-[8.75rem] animate-slide-up delay-200 group">
        <div className="flex justify-between items-start">
        ...
        <div>
          <div className="text-3xl font-display font-medium text-white">
            {todayStats
              ? ((todayStats.input_tokens + todayStats.output_tokens) / 1000000).toFixed(1) + 'M'
              : '0'}
          </div>
      ...
      <div className="bento-card p-6 flex flex-col justify-between h-[8.75rem] animate-slide-up delay-300 group ring-1 ring-neonGreen/20">
        <div className="flex justify-between items-start">
        ...
        <div>
          <div className="text-3xl font-display font-medium text-white">
            {todayStats ? (todayStats.cache_hit_rate * 100).toFixed(1) : '0'}%
          </div>
          <div className="mt-3 text-xs text-neonGreen font-mono border-t border-border/50 pt-2 flex items-center gap-2">
            今日节省 ¥{todayStats ? (todayStats.cost_usd * todayStats.cache_hit_rate * 7.2).toFixed(2) : '0.00'}
          </div>
        </div>
        <div className="absolute inset-0 bg-neonGreen/5 pointer-events-none"></div>
      </div>

      <div className="bento-card p-6 flex flex-col justify-between h-[8.75rem] animate-slide-up delay-400 group">
        <div className="flex justify-between items-start">
        ...
        <div>
          <div className="text-3xl font-display font-medium text-white">
            {latestActivity?.session_count ?? stats?.total_sessions ?? 0}
          </div>
          <div className="flex items-center gap-2 mt-2">
